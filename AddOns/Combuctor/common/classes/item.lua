...
function Item:UpdateBorder()
	...
	if id then
		...
		elseif Addon.sets.glowSets and Search:InSet(link) then
	  	r,g,b = .2, 1, .8
		elseif Addon.sets.glowQuality and quality and tonumber(quality) > 1 then -- 暫時修正
	...
	self.JunkIcon:SetShown(Addon.sets.glowPoor and tonumber(quality) == 0 and not self.info.worthless) -- 暫時修正

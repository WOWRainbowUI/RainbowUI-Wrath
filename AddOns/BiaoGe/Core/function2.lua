...
do
    ...
    function BG.SetListzhuangbei(self)
        ...
        if self.hopenandu then
            text:SetText(BG.STC_b1(L["（SHIFT+点击发送装备，CTRL+点击查看该部位的其他可选装备）"]))
        else
            text:SetText(BG.STC_b1(L["（ALT+点击关注装备，SHIFT+点击发送装备，CTRL+点击查看该部位的其他可选装备）"]))
        end
        -- 下拉列表
        BG.ZhuangbeiList = {}
        local frameright
...
------------------跳转装备库相同部位------------------
function BG.Once(dt, func)
    if BiaoGe and BiaoGe.options and BiaoGe.options.SearchHistory then
        if not BiaoGe.options.SearchHistory["FBCDchoice" .. dt] then
            func()
            BiaoGe.options.SearchHistory["FBCDchoice" .. dt] = true
        end
    end
end

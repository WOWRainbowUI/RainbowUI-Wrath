...
frame:SetScript("OnEvent", function(self, event, addonName)
    ...
    if BG.IsVanilla() then
        ...
        BG.Once("240106" .. RealmId .. player, function()
            local i = 1
            while BiaoGe.FilterClassItemDB[RealmId][player][i] do
                BiaoGe.FilterClassItemDB[RealmId][player][i].ShuXing["命中"] = nil
                BiaoGe.FilterClassItemDB[RealmId][player][i].ShuXing["暴击"] = nil
                i = i + 1
            end
        end)
    else

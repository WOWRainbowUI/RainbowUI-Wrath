...
frame:SetScript("OnEvent", function(self, event, addonName)
    ...
    do
        ...
        do
            ...
            function Y.SearchButtonOnClick()
                ...
                local previous_time = current_time - (Y.searchLastDay * 86400) -- 计算90天前的时间戳
                ...
                BG.OnUpdateTime(function(self, elapsed)
                    ...
                    local time = format("%.1f", Y.lateTime + 0.8 - self.timeElapsed)
                    ...
                    if tonumber(time) <= 0 then
                        ...
                        if sum ~= 0 then
                            ...
                            local link = "|cff00BFFF|Hgarrmission:" .. "BiaoGeYY:" .. L["详细"] .. ":" .. yy ..
                                "|h[" .. L["详细"] .. "]|h|r"
    ...
    do
        local function ChangSendLink(self, even, msg, player, l, cs, t, flag, channelId, ...)
            ...
            -- local yykey = "YY%s*:*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d*%s*%d+"
            local last
    ...
    do
        ...
        function Y.IsLeader(playerName)
            if BG.raidRosterInfo and type(BG.raidRosterInfo) == "table" then
                for index, v in ipairs(BG.raidRosterInfo) do
                    if v.rank == 2 and v.name == playerName then
                        return true
                    end
                    if v.isML and v.name == playerName then
                        return true
                    end
                end
            end
        ...
        f:SetScript("OnEvent", function(self, even, msg, playerName, ...)
            ...
            for yy in string.gmatch(msg, yykey) do
                ...
                for k, v in pairs(BiaoGe.YYdb.LeaderYY) do
                    if v.yy == cleanedYY and v.name == playerName then
                        return
                    end
                end
        ...
        function BG.LiseYY(edit)
            ...
            LibBG:EasyMenu(channelTypeMenu, dropDown, edit, 0, 0, "MENU", 3)
        end
    end
    ...
    do
        ...
        do
            ...
            BG.EndPJ.new = f
            BG.EndPJ.new:SetScript("OnShow", function()
            ...
            do
                ...
                edit:SetScript("OnKeyDown", function(self, enter)
                    if enter == "ENTER" then
                        BG.EndPJ.new.buttonsaveOnClick()
                    end
                end)
                edit:SetScript("OnMouseDown", function(self, enter)
                    if enter == "RightButton" then
            ...
            do
                ...
                bt:SetScript("OnEnter", function(self)
                    local text = L["|cffffffff< 详细评价 >|r\n\n1、在金团表格里写详细评价"]
                    for key, value in pairs(BiaoGe.YYdb.all) do
                        if BG.EndPJ.new.yy:GetText() == value.yy then
                            text = L["|cffffffff< 修改评价 >|r\n\n1、该YY号已有评价，去金团表格里修改评价"]
                        end
                    end
                    GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT", 0, 0)
                    GameTooltip:ClearLines()
        ...
        BG.RegisterEvent("ENCOUNTER_END", function(self, _, bossId, _, _, _, success)
            if bossId ~= id or success ~= 1 or BiaoGe.YYdb.share ~= 1 then
                return
            end
...
f:SetScript("OnEvent", function(self, even, ...)
    ...
    for i, v in pairs(BiaoGe.YYdb.all) do
        if tonumber(yy) == tonumber(v.yy) then
            ...
            local time = random(0.1, Y.lateTime)
...
f:SetScript("OnEvent", function(self, even, isLogin, isReload)
    ...
    if not joined and BiaoGe.YYdb.share == 1 then
...
f:SetScript("OnEvent", function(self, even, isLogin, isReload)
    ...
    --         JoinChannelByName(YY)
    --     end
    --     BG.YYMainFrame.search.button:SetEnabled(true)
    --     BG.YYMainFrame.search.button:SetText(L["查询"])
    --     BG.YYMainFrame.search.cd = 0
    -- end)

...
local function shouldMove(frame)
	...
	if frameCannotBeModified(frame) then
		print("|cffFFC125移動和縮放視窗:|r 戰鬥中無法移動 " .. frame:GetName() .. " during combat.")
		return false
	end
...
local function shouldScale(frame)
	...
	if frameCannotBeModified(frame) then
		print("|cffFFC125移動和縮放視窗:|r 戰鬥中無法縮放 " .. frame:GetName() .. " during combat.")
		return false
	end
...
function DriftHelpers:HandleSlashCommands(msg, editBox)
	...
	elseif (cmd == "reset") then
		...
		print("|cffFFC125移動和縮放視窗:|r 未知的指令 '" .. cmd .. "'")
		DriftHelpers:PrintAllowedCommands()
	end
end
